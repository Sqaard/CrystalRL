"""CRYSTAL-1 B1 v2 — the uniqueness-tracks-belief-fidelity test.
...
MODEL = ROOT / "src/crystal/crystal1_b1v2_ppo.zip"
OUT = HERE / "crystal1_b1_v2_report.json"
N_SSL = 2000
EPOCHS = 600
...
def main():
    ...
    print(f"[B1v2] L1: p_stay={np.diag(T_l).round(3)} E_burst={E_l[:,1].round(3)} | belief MAE vs analytic = {mae:.4f} (v1: 0.024)")

    # ---- L2 v2 ----
    env = Monitor(LearnedBeliefEnv(T_l, E_l, p0_l, tox, seed=0))
    if MODEL.exists():
        model = PPO.load(str(MODEL), env=env, device="cpu"); print("[B1v2] loaded PPO")
    else:
    ...
    report = {"L1_v2": {"n_ssl": N_SSL, "belief_MAE_vs_analytic": round(mae, 4), "v1_MAE": 0.024},
              "returns": {"CRYSTAL1_v2": round(ret, 2), "noise_belief": round(ret_noise, 2), "v1": 6.84, "privileged": 7.11},
              ...
              "gates": gates, "B1v2_PASS": all(gates.values()),
