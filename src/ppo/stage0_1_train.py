...
from src.data.dow30_sectors import get_sector_map, register_single_group_map
...
from src.ppo.dirichlet_policy import (
    ...
    TwoLevelRootStockBetaDirichletActorCriticPolicy,
)
from src.ppo.instrumented_ppo import InstrumentedPPO
from src.ppo.pretrain_teachers import pretrain_policy_from_teachers
from src.ppo.vec_env_factory import make_vec_env
...
def stock_group_ids_from_indices(stock_dim: int, group_indices: list[list[int]]) -> list[int]:
    group_ids = [-1] * int(stock_dim)
    for group_id, group in enumerate(group_indices):
        for idx in group:
            if idx < 0 or idx >= stock_dim:
                raise ValueError(f"group index {idx} is outside stock_dim={stock_dim}")
            if group_ids[idx] != -1:
                raise ValueError(f"stock index {idx} appears in multiple groups")
            group_ids[idx] = int(group_id)
    missing = [idx for idx, group_id in enumerate(group_ids) if group_id < 0]
    if missing:
        raise ValueError(f"group_indices miss stock indices: {missing}")
    return group_ids
...
def policy_for_variant(config: dict[str, Any], variant: dict[str, Any], panel: WeightPanel):
    ...
    if variant["policy_kind"] in {
        ...
        "twolevel_root_stock_shared_scorer",
        ...
        if variant["policy_kind"] == "twolevel_root_stock_shared_scorer":
            two_level_cfg = {**base_policy_cfg.get("two_level", {}), **variant_policy_cfg.get("two_level", {})}
            root_raw_window_cfg = dict(two_level_cfg.get("root_raw_window", {}))
            root_raw_window_days = (
                int(root_raw_window_cfg.get("days", 0))
                if bool(root_raw_window_cfg.get("enabled", False))
                else 0
            )
            ...
            policy_kwargs.update(
                {
                    "feature_columns": panel.feature_columns,
                    "root_feature_names": list(two_level_cfg.get("root_feature_names", [])),
                    "stock_feature_names": list(two_level_cfg.get("stock_feature_names", [])),
                    "root_latent_dim": int(two_level_cfg.get("root_latent_dim", 32)),
                    "root_hidden_dim": int(two_level_cfg.get("root_hidden_dim", 64)),
                    "stock_hidden_dim": int(two_level_cfg.get("stock_hidden_dim", 64)),
                    "stock_group_ids": stock_group_ids_from_indices(len(panel.tickers), group_indices),
                    "ticker_embedding_dim": int(two_level_cfg.get("ticker_embedding_dim", 0)),
                    "asset_relation_mode": str(two_level_cfg.get("asset_relation_mode", "group_one_hot")),
                    "root_raw_window_feature_names": list(root_raw_window_cfg.get("feature_names", [])),
                    "root_raw_window_days": root_raw_window_days,
                }
            )
            return TwoLevelRootStockBetaDirichletActorCriticPolicy, policy_kwargs
...
def evaluate_model(
    ...
    while not done:
        ...
        q_critic_value = model.predict_q(obs, action) if getattr(model, "q_critic_enabled", False) else np.nan
        ...
        if isinstance(daily_steps, list) and daily_steps:
            for daily_info in daily_steps:
                daily_info = dict(daily_info)
                daily_info["q_critic_value"] = q_critic_value
                append_eval_info_row(rows, panel, daily_info, float(daily_info.get("reward", reward)))
            continue
        info = dict(info)
        info["q_critic_value"] = q_critic_value
...
def train_one(
    ...
    *,
    ...
    _ar_cfg = (variant_for_fold.get("reward", {}).get("action_relabel")
               or config.get("environment", {}).get("reward", {}).get("action_relabel")
               or {})
    action_relabel_enabled = bool(_ar_cfg.get("enabled", False))
    model = InstrumentedPPO(
        policy,
        train_env,
        ...
        q_critic_config=variant_for_fold.get("q_critic", config.get("q_critic", {})),
        action_relabel_enabled=action_relabel_enabled,
        **ppo_cfg,
    )
    if action_relabel_enabled:
        print(f"[action-relabel] ENABLED (target={_ar_cfg.get('target', 'target')}) — PPO will train on the "
              f"controller-executed action, not the raw sampled action.")
    model.set_logger(sb3_logger)
    pretrain_summary = pretrain_policy_from_teachers(
        model=model,
        config=config,
    ...
    if getattr(model, "q_critic_enabled", False):
        model.save_q_critic(run_dir / "q_critic.pt")
    train_env.close()
    ...
    metadata = {
        ...
        "q_critic_enabled": bool(getattr(model, "q_critic_enabled", False)),
        "q_critic_path": str(run_dir / "q_critic.pt") if getattr(model, "q_critic_enabled", False) else None,
        "pretrain_summary": pretrain_summary.as_dict(),
...
def summarize_results(results: list[dict[str, Any]], out_root: Path) -> pd.DataFrame:
    ...
    # --- firewall (audit C1): deflate the selection for multiple-testing and flag held-out
    # credibility. Purely ADDITIVE — adds columns, does not change the existing ranking. ---
    try:
        from src.evaluation import firewall as fw
    except Exception:  # firewall not bundled (e.g. a minimal cloud package) -> degrade, don't crash
        fw = None

    sr = grouped["mean_validation_sharpe"].to_numpy(dtype=float)
    n_variants = int(np.isfinite(sr).sum())
    if fw is not None and n_variants >= 2:
        sr_sd = float(np.nanstd(sr, ddof=1))
        noise_floor = float(np.nanmean(sr)) + fw.expected_max_sharpe(sr_sd, n_variants)
    else:
        noise_floor = float("nan")
    grouped["selection_noise_floor"] = noise_floor
    grouped["above_selection_noise_floor"] = grouped["mean_validation_sharpe"] > noise_floor

    # Held-out confirmation: pick the winner on a SELECTION subset of folds, then report its
    # Sharpe on the DISJOINT confirmation folds (never the selection max). A variant is only
    # credible if it both clears the noise floor AND confirms out-of-selection.
    folds_sorted = sorted(df["fold"].dropna().astype(str).unique()) if "fold" in df.columns else []
    if len(folds_sorted) >= 2:
        half = max(1, len(folds_sorted) // 2)
        sel_folds, conf_folds = set(folds_sorted[:half]), set(folds_sorted[half:])
        fold_str = df["fold"].astype(str)
        sel_mean = df[fold_str.isin(sel_folds)].groupby("variant")["sharpe"].mean()
        conf_mean = df[fold_str.isin(conf_folds)].groupby("variant")["sharpe"].mean()
        grouped = grouped.merge(sel_mean.rename("selection_fold_sharpe"), on="variant", how="left")
        grouped = grouped.merge(conf_mean.rename("confirmation_fold_sharpe"), on="variant", how="left")
        # ABSOLUTE held-out bar (not a relative median, which passes ~50% by construction): a
        # variant must stay above MIN_CONFIRM_SHARPE on the folds it was NOT selected on.
        MIN_CONFIRM_SHARPE = 0.0
        grouped["held_out_confirmed"] = grouped["confirmation_fold_sharpe"] > MIN_CONFIRM_SHARPE
        # Also single out the actual selection WINNER and confirm only it (fw.held_out_confirmation).
        if fw is not None and not sel_mean.empty:
            hoc = fw.held_out_confirmation(sel_mean.to_dict(), conf_mean.to_dict(), MIN_CONFIRM_SHARPE)
            grouped["is_selection_winner"] = grouped["variant"] == hoc["winner"]
            grouped["selection_winner_confirmed"] = bool(hoc["confirmed"])
    else:
        grouped["selection_fold_sharpe"] = float("nan")
        grouped["confirmation_fold_sharpe"] = float("nan")
        grouped["held_out_confirmed"] = False
    grouped["firewall_credible"] = grouped["above_selection_noise_floor"] & grouped["held_out_confirmed"]
...
def register_universe_sector_map(config: dict[str, Any]) -> None:
    """Wire an arbitrary-universe single-group sector map through ``get_sector_map`` at config load.

    The env validates every panel ticker against the configured ``universe.sector_map`` (an in-memory
    registry), so a non-Dow universe (e.g. Qlib csi300 SH600000/... names) must register its map before
    any env is built. If the config sets ``universe.sector_map_register: {from_model_ready_csv: true,
    group: "U"}`` we read the panel tickers straight from ``data.model_ready_csv`` and register a
    single-group ("all-U") map under the name in ``universe.sector_map``. No-op for known maps.
    """
    universe_cfg = config.get("universe", {})
    reg_cfg = universe_cfg.get("sector_map_register")
    if not reg_cfg:
        return
    map_name = str(universe_cfg.get("sector_map", "dow30_static"))
    group = str(reg_cfg.get("group", "U"))
    csv_path = resolve(reg_cfg.get("model_ready_csv") or config["data"]["model_ready_csv"])
    tickers = sorted(pd.read_csv(csv_path, usecols=["tic"])["tic"].astype(str).unique().tolist())
    register_single_group_map(map_name, tickers, group=group)
    print(f"[universe] registered single-group sector map '{map_name}' "
          f"({len(tickers)} tickers -> group '{group}') from {csv_path.name}")


def main() -> None:
    args = parse_args()
    config = load_yaml(resolve(args.config))
    register_universe_sector_map(config)
