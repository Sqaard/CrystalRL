...
seeds); $F{=}1.00$ (belief-write fidelity on the real daily panel, \S\ref{sec:levers});
$\Delta_{\text{MDL}}{=}0.0$: $\textsc{CrystalScore}{=}0.92$ (Figure~\ref{fig:crystalscore}).
The entire $6\times$ gap is Simulatability --- exactly what ``born legible'' buys.
...
families of influence on CRYSTAL-1's PPO heads, all on the real US daily panel (train 2010--18,
held-out evaluation after), and report one measured outcome per lever
...
moves the drawdown dial reliably \citep{ng1999shaping}: a $5\%$ budget delivers $-4.81\%$
held-out max drawdown; a $10\%$ budget $-10.4\%$. The honesty rider: the cold heads converge to
near-\emph{constant} exposure dials ($0.25/0.5/0.75$) --- a drawdown-matched twin shows
$z = 0.00$ timing skill. On this substrate reward shaping bought \emph{risk placement}, not
timing, and cold PPO re-derived from scratch what the goal machinery already knew: the dial
beats timing.

\paragraph{T --- teachers.} The cautionary tale first: the cold heads' argmax behavior looked
sensible while the underlying preferences were nearly flat --- an effectively untrained network
masked by deterministic evaluation. (For a behavioral scientist: inferring function from the
argmax of a flat tuning curve.) The cure is a teacher: behavior cloning from the certified
readable rule (Eq.~\ref{eq:rule}),
\begin{equation}
L_{\text{BC}}(\theta) \;=\; -\,\E_{(s,a^{*})}\big[\log \pi_\theta(a^{*}\mid s)\big],
\label{eq:bc}
\end{equation}
reaching $87\%$ agreement, used as a warm start --- after which PPO fine-tuning \emph{keeps}
the teacher's behavior (warm $=$ BC). The same mechanism closed the soft-tree parity gap
(cold $-2.57 \to$ annealed $-0.27 \to$ BC-warm-started $-0.018$, paired $p\approx0.84$ --- full
parity): imitation as initialization, optimization as refinement
\citep{ross2011dagger,rusu2016distillation}.

\paragraph{I --- interventions.} Belief writes (Eq.~\ref{eq:write}) are exercised directly on
the real-panel machinery: write fidelity $1.0$ (the commanded belief is the realized belief),
and a $\pm2$-nat dose slides the agent between named behavior codes. The result we are proudest
of is a refusal: a $+1$-nat promotion whose fine-tuned policy failed the non-inferiority test
on the certified objective,
\begin{equation}
\text{accept the write} \iff \widehat{J}_{\text{post}} \ \ge\ \widehat{J}_{\text{pre}} -
\delta_{\text{NI}},
\label{eq:ni}
\end{equation}
was \emph{blocked}. An intervention surface that cannot say no is a jailbreak surface.

\paragraph{A --- architecture.} Tree depth is a legibility dial set \emph{before} training:
depth-2 yields a 4-leaf policy, depth-3 the 8-leaf one --- story size traded against
expressiveness ahead of time, not reconstructed after.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_levers.png}
  \caption{The four levers on the real daily panel, one panel each. R (Eq.~\ref{eq:reward}):
  realized held-out max drawdown lands on the budget written into the reward. T
  (Eq.~\ref{eq:bc}): cold PPO's flat-preference argmax (agreement $0.02$) is cured by the
  certified-rule teacher ($0.87$) and \emph{persists} through PPO fine-tuning. I
  (Eqs.~\ref{eq:write},~\ref{eq:ni}): writes land with fidelity 1.0; the $+1$-nat promotion is
  REFUSED by the non-inferiority bar. A: depth as a pre-training legibility dial.}
  \label{fig:levers}
\end{figure}
